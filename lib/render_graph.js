...
function renderHouse(data, house, path) {
    ...
    Object.keys(people).forEach((key, idx) => {
        peopleMeat.push(
            renderPerson(data, house, people[key] || {}, path.concat([key]))
        );
    });

    const meat = [
        // People and families
        families.map((home, idx) =>
            renderFamily(data, house, home || {}, path.concat([idx]))
        ),
        peopleMeat,
    ];
...
function renderPerson(data, house, person, path) {
    ...
    if (
        ...
        if (person.born || person.died) {
            if (person.born && person.died) {
                lifespan = `﹡${formatDate(person.born)} — †${formatDate(
                    person.died
                )}`;
            } else if (person.born) {
